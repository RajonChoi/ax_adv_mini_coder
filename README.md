...
### 6. **웹 UI (Flask)**
   - 모던한 웹 인터페이스로 작업 요청 입력
   - 실시간 작업 실행 및 결과 표시
   - 모바일 반응형 디자인

### 7. **자동 프로젝트 관리**
   - 매 실행마다 고유한 프로젝트명 자동 생성 (UUID 기반)
   - 프로젝트 폴더 및 초기 파일(`app.py`) 자동 생성
   - 프로젝트 격리를 통한 작업 관리 편의성
    ...
    └── 📁 agents/                  # 에이전트 패키지
        ...
        └── 📄 state_models.py      # 상태 관리 클래스
            └── WorkspaceState      # LangGraph 상태 정의
                ├── messages        # 대화 기록 (Annotated 리듀서 사용)
                ├── current_workspace_state # 파일 상태
                ├── error_logs      # 에러 로그
                ├── project_name    # 자동 생성된 프로젝트명
                ├── add_message()
                ├── add_error()
                └── project_store() # 파일 스냅샷
```
...
웹 UI에서:
- 작업 요청을 입력창에 입력
- "입력" 버튼 클릭 (또는 Ctrl+Enter)
- 결과를 깔끔한 창으로 확인

### 예제 1: 새 모듈 생성

**입력 예시:**
> "utils.py 모듈을 만들어서 문자열 유틸리티 함수 3개 구현"
...
**입력 예시:**
> "app.py의 라인 45에서 IndexError가 발생해. 예외 처리를 추가해"

### 예제 3: 코드 리팩토링

**입력 예시:**
> "main.py를 리팩토링해서 함수를 분리하고 가독성을 좋게 해"
...
- **입력창**: 코딩 작업 요청을 자연어로 입력할 수 있는 큰 textarea
- **입력 버튼**: 작업 실행 (Ctrl+Enter 단축키도 지원)
- **초기화 버튼**: 입력창 내용 초기화
- **결과 표시**: 에이전트 실행 결과를 깔끔한 박스로 표시
- **프로젝트명**: 자동으로 생성된 프로젝트 ID 표시 (예: `project_a1b2c3d4`)
- **로딩 애니메이션**: 작업 실행 중 시각적 피드백 제공
...
// 사용
executeTask('Python으로 간단한 계산기 만들기');
```

#### 프로젝트 자동 생성

매번 웹 UI를 통해 작업을 실행할 때:

1. **고유 프로젝트명 자동 생성**: `project_xxxxxxxx` (아래 12자 UUID 해시)
2. **프로젝트 폴더 자동 생성**: `{project_root}/{project_name}/`
3. **초기 파일 생성**: `{project_name}/app.py` (프로젝트 진입점)
4. **결과 화면에 프로젝트명 표시**
...
```bash
OPENROUTER_API_KEY=sk-or-v1-... python web_app.py
# 이후 브라우저에서 작업 요청
```
...
```bash
# 이미지 빌드
docker build -t jrue-coder:latest .
...
# 에이전트 작업 실행
docker compose exec jrue-coder python web_app.py
# 브라우저에서 작업 설명 입력
